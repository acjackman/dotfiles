...
- **`,doctor-herdr`** — covers both ways a `brew upgrade` breaks a running herdr server. Always runs `herdr server reload-config`, which re-registers `[[keys.command]]` custom keybindings the old server may have silently dropped (non-destructive, so it applies by default). Also detects a client/server **protocol mismatch**, where every plugin and all agent surface control break at once. **Exception to the "applies fixes by default" rule:** the only fix for the mismatch is restarting the server, which kills every pane process including running agents, so it reports by default and restarts only under `--restart`. Exits 1 when a mismatch needs fixing.
