...
2. **Before editing, check if the file is generated by a script**
   ...
   - Example: `dot_config/tuna/run_onchange_after_generate_config.py.tmpl` generates `~/.config/tuna/config.toml`
   - Example: `dot_claude/run_onchange_after_generate_settings.sh.tmpl` generates `~/.claude/settings.json` from `permissions.json`
...
3. **Use Glob/Grep in the chezmoi directory for exploratory searches**
